...
export async function saveGameResult(uid, result) {
  await addDoc(collection(db, 'games'), { uid, ...result, playedAt: serverTimestamp() });
...
export async function createGroup(uid, name, isPublic = true) {
  ...
  const ref = await addDoc(collection(db, 'groups'), {
    name, code, isPublic, isAuto: false, ownerId: uid,
    members: [uid], totalPoints: 0,
...
export async function assignAutoGroup(uid, levelId) {
  const q = query(collection(db, 'groups'), where('isAuto', '==', true), where('level', '==', levelId), limit(5));
  const snap = await getDocs(q);
  let groupId = null;
  for (const d of snap.docs) {
    if (d.data().members.length < 15) { groupId = d.id; break; }
  }
  if (!groupId) {
    const ref = await addDoc(collection(db, 'groups'), {
      name: `Auto Group ${Date.now()}`, isAuto: true, isPublic: false,
      level: levelId, members: [], totalPoints: 0,
      createdAt: serverTimestamp(), updatedAt: serverTimestamp(),
    });
    groupId = ref.id;
  }
...
export async function getLeaderboard(type = 'global', limit_ = 50) {
  ...
  const q = query(collection(db, col), orderBy('totalPoints', 'desc'), limit(limit_));
  const snap = await getDocs(q);
  return snap.docs.map((d, i) => ({ rank: i + 1, ...d.data(), id: d.id }));
}

export function subscribeLeaderboard(groupId, cb) {
  const q = query(collection(db, 'users'), where('groupId', '==', groupId), orderBy('totalPoints', 'desc'), limit(20));
  return onSnapshot(q, snap => cb(snap.docs.map((d, i) => ({ rank: i + 1, ...d.data(), id: d.id }))));
}

export async function getCurrentTournament() {
  const now = new Date();
  const q = query(collection(db, 'tournaments'), where('endsAt', '>', now), orderBy('endsAt'), limit(1));
  const snap = await getDocs(q);
